...
ARCS = [
    ...
    ((2, 0), (2, 10), "Arc VIII - Synthesis and self-audit (v2.00-10): Bayesian comparison, falsifiers, robustness"),
...
def parse(path):
    ...
    ver = version_key(ver_str) if ver_str else (0, 5 if "RESEARCH-REPORT" not in base else 9)
...
def main():
    ...
    notes.sort(key=lambda n: (n["ver"], n["date"], n["file"]))
    ...
    for lo, hi, label in ARCS:
        if lo[0] == 0:   # Foundations: all pre-June notes, regardless of version
