...
scenario) → [v2.440](results/2026-07-02-v2.440-qnm-emergent-string.md). **The scale-clean fingerprint discriminates that dichotomy (v2.474):** a Regge (string) tower and a KK tower have *different* scale-clean double-ratios (1.23 vs 1.05, since m²~n vs m~n), so the fingerprint distinguishes tower *type* (the handle v2.438 said was missing). **Executed (v2.476–477), honestly inconclusive:** the *bosonic* Veneziano forward residues are provably flat (Γ-function telescoping) → fingerprint ζ(2)ζ(4)/ζ(3)² = 1.232, matching the candidate's 1.32 to ~7% — but the bosonic string is tachyonic; the physical *open superstring* has a SUSY-shifted factor with residues r_n = 1/n → ζ(3)ζ(5)/ζ(4)² = 1.064 (~24% off), and additionally a kinematic prefactor (uncomputed). So the exact-string match is **amplitude-dependent and not established** — not string-inconsistent, but not a clean pass. The heterotic ID rests on its *other* supports (required R², parity/Green–Schwarz, string-like tower *shape*). → [v2.476](results/2026-07-08-v2.476-qnm-exact-veneziano-fingerprint.md), [v2.477](results/2026-07-08-v2.477-qnm-superstring-fingerprint-correction.md). **A scale-clean handle on the embedding (v2.464–466):** the individual curvature ratios need the (unknown) string scale, but every moment-tower *double-ratio* (g_{k−1}g_{k+1})/g_k² has α′ **cancel** — a pure number, so the candidate has a scale-clean UV **fingerprint** a string embedding must reproduce without the string scale. Executing it with a toy flat-residue Regge tower (moments = Riemann ζ-values) gives {1.23, 1.06} vs the candidate's {1.32, 1.00} — string-like in *shape* (>1, decreasing toward 1), a currently-*passing* check at the toy-Regge level → [v2.466](results/2026-07-03-v2.466-qnm-regge-fingerprint-match.md). *(Proof of concept: the exact Virasoro–Shapiro value is uncomputed; the shape is robust, the value approximate.)*
