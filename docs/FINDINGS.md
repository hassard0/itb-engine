...
scenario) → [v2.440](results/2026-07-02-v2.440-qnm-emergent-string.md). **The scale-clean fingerprint discriminates that dichotomy (v2.474):** a Regge (string) tower and a KK tower have *different* scale-clean double-ratios (1.23 vs 1.05, since m²~n vs m~n), so the fingerprint distinguishes tower *type* (the handle v2.438 said was missing). **Settled honestly (v2.478):** the candidate's double-ratio is *not sharply predicted* — its feasible range over the rigorous region is **[1.0, ~11]** (bounded only below by the moment-tower floor). The string values (bosonic Veneziano 1.23, superstring 1.06) lie *within* it near the lower edge, so the candidate is fingerprint-**consistent** with a string embedding but the fingerprint does **not** predict a value or discriminate string-vs-KK — the earlier point-comparisons (matches ~7%, favors Regge; v2.474/v2.476) over-read the single constructed point (1.32, itself Chebyshev-contaminated). The scale-independent double-ratio remains a genuine *diagnostic* (v2.464); the heterotic ID rests on its *other* supports (required R², parity/Green–Schwarz, string-like tower *shape*). → [v2.478](results/2026-07-08-v2.478-qnm-fingerprint-feasible-range.md). **Positively (v2.480):** the family *contains an explicit feasible member* whose matter moment ratios exactly realize the superstring forward spectrum (a_k ∝ ζ(k+1)) — so it is string-**realizable** (a concrete Regge-tower member sits inside the consistency region), not merely consistent; existence, not uniqueness (the center is a different point) → [v2.480](results/2026-07-08-v2.480-qnm-explicit-string-member.md). And a *single* point is string-realizable in **both** sectors to good approximation (matter exact, curvature ~93%; the curvature slightly more gapped, likely because the open-string ζ(k+1) is the wrong target for the closed/gravity sector) — a **nearly-fully-stringy** consistent QG EFT → [v2.481](results/2026-07-08-v2.481-qnm-fully-stringy-member.md). The curvature's extra gap is capped by the **rigorous** `graviton_forward_positivity` bound — gravity-sector unitarity forces a more-gapped (closed-string-like) curvature tower, the heterotic two-sector structure (open-string matter + closed-string gravity), turning the residual into a *feature* → [v2.482](results/2026-07-08-v2.482-qnm-curvature-gap-physics.md). **A scale-clean handle on the embedding (v2.464–466):** the individual curvature ratios need the (unknown) string scale, but every moment-tower *double-ratio* (g_{k−1}g_{k+1})/g_k² has α′ **cancel** — a pure number, so the candidate has a scale-clean UV **fingerprint** a string embedding must reproduce without the string scale. Executing it with a toy flat-residue Regge tower (moments = Riemann ζ-values) gives {1.23, 1.06} vs the candidate's {1.32, 1.00} — string-like in *shape* (>1, decreasing toward 1), a currently-*passing* check at the toy-Regge level → [v2.466](results/2026-07-03-v2.466-qnm-regge-fingerprint-match.md). *(Proof of concept: the exact Virasoro–Shapiro value is uncomputed; the shape is robust, the value approximate.)*
...
**The scale-independent core (v2.451–454) — predictions with *no* toy scales.** The sharpest, cleanest part of the program: the candidate touches real data through *dimensionless* relations needing neither the string scale nor any O(1)-toy coefficient. (1) **Cosmic birefringence β ~ α_EM**: the heterotic axion's EM anomaly coupling gives β = (c_γ α_EM/4π)·Δθ (scale-independent; the axion must be a QCD-decoupled *photophilic ALP* to stay ultralight, so c_γ is model-dependent — [v2.468](results/2026-07-03-v2.468-qnm-strong-cp.md)), so β ~ α_EM/4π × O(1–10) ~ 0.03–0.3°, matching the measured 0.34° in order of magnitude (β being *of order α_EM* is the anomaly-axion fingerprint) → [v2.451](results/2026-07-02-v2.451-qnm-alpha-em-birefringence.md). (2) **Inflation consistency line r = 3(1−n_s)²** pins r ≈ 0.0037 from the measured n_s (n_t, α_s follow from the same N) — LiteBIRD-testable → [v2.452](results/2026-07-02-v2.452-qnm-ns-r-consistency.md), [v2.453](results/2026-07-02-v2.453-qnm-inflation-consistency-set.md). *(This r ≈ 0.0037 sits ~28 orders above the Trans-Planckian Censorship ceiling, so the candidate **violates the TCC** even while it obeys refined-dS / distance / ESC — a non-uniform swampland-consistency; the TCC is contested and a LiteBIRD r-detection would disfavor it, not the candidate → [v2.472](results/2026-07-08-v2.472-qnm-tcc-inflation-tension.md). This is a **pattern**: the candidate also violates the axion-WGC (its DE axion needs instanton action S~276 vs S≲1), and the two *aggressive* swampland conjectures it violates are exactly the two that would forbid its two *observable* predictions (r via LiteBIRD, dynamical w via DESI) — so its observable QG predictions live at the aggressive-swampland edge and arbitrate those conjectures → [v2.473](results/2026-07-08-v2.473-qnm-swampland-pattern.md).)* (3) **Dark-energy thawing line wa ≈ −1.5(1+w0), w ≥ −1** — the candidate's *most vulnerable* front: it matches DESI's w0 > −1 sign but predicts *milder* evolution than DESI's steep central wa / CPL phantom-past hint (a trend-tension, falsifiable by a robust w < −1) → [v2.454](results/2026-07-02-v2.454-qnm-dark-energy-thawing-line.md). *Its honest cosmological-tension scorecard*: it does **not** resolve H₀ (a canonical axion cannot do phantom or early dark energy — [v2.467](results/2026-07-03-v2.467-qnm-h0-tension.md)) but **mildly helps** S8 (w > −1 suppresses structure growth, lowering σ8 — [v2.469](results/2026-07-08-v2.469-qnm-s8-tension.md)) — opposite signs (distance vs growth), so the thawing axion is *not uniformly disfavored*. *All three are plateau-/class-level (they validate the class, not uniquely this candidate), but they are the program's most scale-clean, directly-testable content.*
...
## 8. The R4/GWOSC/LALSuite live-data frontier (parked)

The foundational program's most-developed live-data route (v2.170–208) turned the R4-through-gravitational-waves path into an executable, *nonclaiming* pipeline: public GWOSC strain ingestion, a LALSuite-calibrated detector response, coarse R4 likelihoods on GW150914 / GW200129, and a hash-pinned pyRing quartic-EFT source audit. It remains **blocked** on one missing ingredient — a source-backed, full-rank qNM→R4 (Bresciani-axis) sensitivity map, which the public literature does not supply (v2.209). The exact remaining blockers are in [ROADMAP.md](ROADMAP.md) (parked route); the per-cycle notes are v2.170–208 in [the index](results/INDEX.md).

## 9. The foundational-era bottom line

*(This summarizes the v1.7x–v2.2x foundational program; the current headline is the ★ section at the top.)* Given amplitude/causality/holographic consistency **plus two ingested experiments**, the engine points to a **parity-violating, screened-scalaron EFT**: it matches cosmic birefringence, requires screening to survive sub-mm gravity, and predicts GW/PTA parity signals just below current reach. The consistent island is thin (~0.6%) and ~3.4-dimensional; **6 measurements** would pin it; its single sharpest tension is the birefringence/sub-mm-gravity clash; and its sharpest near-future test is next-gen CMB birefringence (LiteBIRD/CMB-S4). All of this is stated with explicit, carried caveats — the engine is built to be honest about the difference between what it has shown and what it has merely sketched.
